...
const removeNullTransitions = (grammar) => {
...
const updateNull = (grammar, nullState) => {
    let cfg = structuredClone(grammar);
    for (let s in cfg) {
        let arr = cfg[s];
        for (let i = 0; i < arr.length; i++) {
            ...
            if (splitedState.includes(nullState)) {
                ...
                if (splitedState.length > 2) {
                    let count = 0;
                    splitedState.map(el => { if (el === nullState)
                        count++; });
                    ...
                    // S -> ABA and A -> epsilon, will add S -> AB | BA | B
                        ...
                        const newCombinations = makeCombinations(splitedState, nullState);
...
const makeCombinations = (s, nullState) => {
    const res = [];
    const idxs = [];
