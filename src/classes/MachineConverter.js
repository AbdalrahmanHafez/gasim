...
export default class MachineConverter {
  constructor(machineType, conversionType) {
    this.machineType = machineType; // its a actually tab type not anything special
    this.conversionType = conversionType;
  }

  convertNFAtoDFA(srcCy, dstCy) {
  ...
  addConversion(srcCy, dstCy) {
    const mt = this.machineType;
    const ct = this.conversionType;
    if (mt === tabTypes.FA && ct === conversionType.NFAtoDFA) {
      this.convertNFAtoDFA(srcCy, dstCy);
    } else {
      throw new Error("MachineConverter: Unknown machine type");
    }
  }
}
